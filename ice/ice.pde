...
void draw() {
  ...
  //bg.filter(INVERT);
  tint(255, 200-intro.volume_Mid*1);
  imageMode(CENTER);
  float rs =1;
  ...
  if (mode == 0) {
    ...
    for (int s=0; s<torii.size(); s++) {
      ...
      stroke(red(P.c), green(P.c), blue(P.c), 0);
      strokeWeight(1);
      ...
      //noStroke();
      colorMode(HSB, 255);
      int foo = floor(random(5));
      color fooC = color(255);
      if (foo==0)fooC = color(#0FA3B1);
      if (foo==1)fooC = color(#D9E5D6);
      if (foo==2)fooC = color(#EDDEA4);
      if (foo==3)fooC = color(#F7A072);
      if (foo==4)fooC = color(#FF9B42);
